...
def test_recursive_reduction_forecaster_global_switches_to_local():
    """Verifies that the forecaster switches from 'global' to 'local' pooling when needed"""
    df = create_mock_data(n_samples=15, hierarchy=False)
    fh = [1, 2, 3]
    forecaster = RecursiveReductionForecaster(estimator=LinearRegression(), window_length=5, pooling="global")
    
    forecaster.fit(df)
    assert forecaster.pooling == "global", "Pooling should initially be 'global'"

    with patch.object(forecaster, "_predict_out_of_sample_v2_local", wraps=forecaster._predict_out_of_sample_v2_local) as mock_local:
        y_pred = forecaster.predict(fh=fh)
        mock_local.assert_called_once(), "Expected _predict_out_of_sample_v2_local to be called"

    assert forecaster.pooling == "local", "Pooling should switch to 'local' after prediction"